# todo
# 3.understand highway network
# 4.implement highway network
# 5.understand attention layer
...
class BiDAF(nn.module):
	def __init__(self, config):
		...
		self.char_embedding_layer = nn.layers.CharEmbedding(\
			self.config.args.char_single_embedding_dim,
			self.config.args.char_embedding_dim, # out_channel_dim
			self.config.args.filter_height,
			self.config.args.dropout,
			self.config.args.char_vocab_size)
		self.word_embedding_layer = nn.Embedding(self.vocab_size, self.config.args.word_embedding_dim)
		self.init_word_embedding()
		self.highway_network = ??? # used to combine character and word embedding
	...
	def init_config(passages, questions, answers):
		self.batch_size = passages.size(0)
	...
	def forward(passages, questions, answers=[]):
		init_config(passages, questions, answers)
		...
		# passages: batch_size * max_p_length (in word_indices)
		...
		p_char_embed = self.char_embedding_layer(passages_char)
		q_char_embed = self.char_embedding_layer(questions_char)
		assert p_char_embed.size() == (self.batch_size, self.max_p_length, self.char_embedding_dim)
		assert q_char_embed.size() == (self.batch_size, self.max_q_length, self.char_embedding_dim)
		
		p_word_embed = self.word_embedding_layer(passages_char)
		q_word_embed = self.word_embedding_layer(questions_char)
		assert p_word_embed.size() == (self.batch_size, self.max_p_length, self.word_embedding_dim)
		...
		p_embed_X = self.highway_network(p_concat_embed)
		q_embed_Q = self.highway_netowrk(q_concat_embed)
		assert p_concat_embed.size() == (self.batch_size, self.max_p_length, self.char_embedding_dim + self.word_embedding_dim)
		assert q_concat_embed.size() == (self.batch_size, self.max_q_length, self.char_embedding_dim + self.word_embedding_dim)
		assert p_embed_X.size() == (self.batch_size, self.max_p_length, self.embedding_dim)
		...
		assert p_context_H.size() == (self.batch_size, self.max_p_length, self.contextual_dim)
		assert q_context_U.size() == (self.batch_size, self.max_q_length, self.contextual_dim)

		### ATTENTION LAYER ###
		G = self.attention_layer(p_context_H, q_context_U)
		assert G.size() == (self.batch_size, self.max_p_length, self.attention_dim)

		### MODELING LAYER ###
		self.model_hidden = self.init_hidden(self.model_dim)
		M, self.model_hidden = self.model_biLSTM(G, self.model_hidden)
		assert M.size() == (self.batch_size, self.max_p_length, self.model_dim)
		

		### OUTPUT LAYER ###
		p_1 = torch.cat((G, M), 2)
		p_1 = self.w_p1(p_1)
		assert p_1.size() == (self.batch_size, self.max_p_length, 1)
		p_1 = p_1.contiguous().view(self.batch_size, self.max_p_length)
		p_1 = nn.Softmax()(p_1)

		self.output_hidden = self.init_hidden(self.model_dim)
		M_2, self.output_hidden = self.output_biLSTM(M, self.output_hidden)
		p_2 = torch.cat((G, M_2), 2)
		p_2 = self.w_p2(p_2)
		p_2 = nn.Softmax()(p_2)
